...
def load_data(dataset):
    ...
    return adj_matrix, features, labels, id_index_map, raw_data_cites


# 随机删减边
def random_edge_sampler(percent, num, id_index_map, raw_data_cites):
    raw_data_cites = raw_data_cites.sample(frac=1 - percent)
    # 创建一个规模和邻接矩阵一样大小的矩阵(2708, 2708)
    adj_matrix = np.zeros((num, num))
    # 创建邻接矩阵
    for thesis, cite_thesis in zip(raw_data_cites[0], raw_data_cites[1]):
